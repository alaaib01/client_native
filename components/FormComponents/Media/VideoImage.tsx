...
const VideoImage = (props: Props) => {
    ...
    const valueChanged = () => {
        dispatch({
            type: STORE_CONSTS.FORM.ACTIONS.ADD_PROP, payload: {
                [props.uid]: {
                    videos: videos,
                    images: image
                }
    ...
    const saveMediaToDevice = async (result: ImagePicker.ImagePickerResult) => {
        ...
        if (asset.mediaType === 'video') {
            setVideos(currstate => [...currstate, { id: asset.id, uri: asset.uri, fileName: asset.filename }])
        }
        else {
            setImage(currstate => [...currstate, { id: asset.id, uri: asset.uri, fileName: asset.filename }]);
        }
    }
    const getMediaFromDevice = async (result: ImagePicker.ImagePickerResult) => {
        if (result.type === 'video') {
            setVideos(currstate => [...currstate, { id: result.id, uri: result.uri, fileName: result.filename }])
        }
        else {
            setImage(currstate => [...currstate, { id: result.id, uri: result.uri, fileName: result.filename }]);
        }
    }
    ...
    return (
        <BaseFormComponent
            ...
            <View>
                ...
                <View style={{ flex: 1, alignItems: 'flex-start', justifyContent: 'flex-start', marginVertical: 15, elevation: 3, flexDirection: 'row' }}>
                    {image.length ? image.map(src => {
                        return (
                            <Thumbnail square key={src.id || src.uri} source={{ uri: src.uri }} style={{ height: 150, width: 150, margin: 10 }} />
                        )
                ...
                <View style={{ flex: 1, alignItems: 'flex-start', justifyContent: 'flex-start', marginVertical: 14, flexDirection: 'column' }}>
                    {videos.length ? videos.map(src => {
                        return (
                            <VidePlayer key={src.id || src.uri} videoSrc={src} />
                        )
                ...
                <View style={styles.btnGroup}>
                    ...
                    <Button style={{ ...styles.btn, ...styles.secondaryBtn }} onPress={pickImageFromGalery}>
                        <Text >העלאה</Text>
